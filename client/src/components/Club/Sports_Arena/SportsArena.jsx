...
const SportsArena = ({ sports, handleButtonClick, theme }) => {
  ...
  const getUniqueArenas = () => {
    const uniqueArenaIds = [...new Set(sports.map((sport) => sport.sports_arena_id))];
    return uniqueArenaIds.map((arenaId) => sports.find((sport) => sport.sports_arena_id === arenaId));
  };
      ...
      {uniqueArenas.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {uniqueArenas.map((arena) => (
            <div key={arena.sports_arena_id} className="bg-white shadow-md rounded-lg overflow-hidden">
              {arena.sports_arena_image ? (
                <img
                  className="w-full h-40 object-cover"
                  src={arena.sports_arena_image}
                  alt={arena.sports_arena_name ? `${arena.sports_arena_name} Image` : "Arena Image"}
                />
              ) : (
              ...
              <div className="p-4">
                <h3 className="text-lg font-semibold mb-2">{arena.sports_arena_name || "No Name"}</h3>
                <div className="flex items-center mb-2">
                  <IoLocationSharp className="text-red-500 mr-2" />
                  <p className="text-gray-600">{arena.sports_arena_location || "No Location"}</p>
                </div>
                <p className="text-gray-500">Address: {arena.sports_arena_address || "No Address"}</p>
              </div>
            </div>
          ))}
        </div>
